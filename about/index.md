...
```yml
  name: 如也 # 站点名，显示在首页、页脚和文章选项卡
  description: 代码如诗 # 站点副标题，显示在页首
  author: Sean Huai # 作者名，当文章没有声明作者时，采用此名
  permalink: /:year/:month/:title # 永久链接，可用参数 `:year/:month/:day/:title`
  backgroundImage: "https://cdn.ruye.org.cn/blog/images/banner.jpg" # 默认页首底图，当文章没有声明背景图时，采用此图
  disqus: ruye # 输入 disqus 用户名或 false，输入用户名时则加载 disqus 评论系统，否则不加载
```
